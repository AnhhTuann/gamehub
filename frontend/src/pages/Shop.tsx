  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 md:px-6 py-8 flex-1 flex flex-col">
      ...
      <div className="text-center mb-10">
        <h1 className="font-pixel text-lg md:text-xl text-gamehub-text mb-3 tracking-wider">
          GAME <span className="text-gamehub-purple">CATALOG</span>
        </h1>
        <p className="text-gamehub-muted text-sm max-w-md mx-auto">
          Explore our curated selection of premium video games across all genres and platforms.
        </p>
      </div>

      {/* Horizontal Filter Navbar */}
      <div className="flex flex-col sm:flex-row items-center justify-between border-b border-gamehub-border pb-4 mb-8 gap-4 sticky top-16 z-30 bg-gamehub-bg/90 backdrop-blur pt-4">
        
        {/* Filters Select Dropdowns */}
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <span className="font-pixel text-[10px] text-gamehub-muted tracking-widest mr-2">FILTERS:</span>
          ...
          {(selectedGenre || selectedPlatform || sortBy !== '-added') && (
            <button
              ...
              className="text-xs text-gamehub-muted hover:text-gamehub-danger transition-colors flex items-center gap-1 font-medium"
            >
        ...
        {/* Total Count */}
        <div className="flex items-center gap-2 mt-4 sm:mt-0 font-pixel text-[10px] tracking-widest text-gamehub-muted">
          <span className="text-gamehub-purple font-bold text-sm">{totalGames.toLocaleString()}</span> GAMES FOUND
        </div>
      </div>
            ...
            <AnimatePresence mode="popLayout">
              {(isLoading && currentPage === 1) ? (
                [...Array(6)].map((_, i) => (
                  <div key={i} className="bg-gamehub-surface border border-gamehub-border rounded-lg animate-pulse overflow-hidden h-full">
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="h-4 bg-gamehub-bg w-2/3 rounded" />
                      <div className="h-3 bg-gamehub-bg w-1/4 rounded" />
                    </div>
                  </div>
                ))
                ...
                <div className="col-span-full text-center py-16">
                  ...
                  <p className="font-pixel text-xs text-gamehub-muted tracking-wider mb-2">NO GAMES FOUND</p>
                  <p className="text-sm text-gamehub-muted">Try adjusting your filters</p>
                </div>
              ) : (
          ...
          {hasMore && (
            <div className="mt-12 mb-8 flex justify-center">
              <button
                ...
                disabled={isLoading}
                className="px-8 py-3 bg-gamehub-purple text-white font-bold text-sm tracking-widest rounded hover:-translate-y-1 hover:opacity-80 transition-all duration-200 disabled:opacity-60 flex items-center gap-2 uppercase select-none cursor-pointer shadow-md"
              >
                {isLoading ? (
                  <>
