...
export const Checkout = () => {
  ...
  if (cart.length === 0 && !isSuccessModalOpen) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-24 text-center bg-gamehub-bg min-h-screen">
        ...
        <Link 
          to="/"
          className="px-8 py-4 bg-gamehub-purple text-black font-pixel text-xs rounded hover:bg-[#ff79c6] hover:-translate-y-1 transition-all duration-200"
  ...
  const Input = ({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) => (
    <div className="flex flex-col gap-2 w-full">
      ...
      <input 
        className="w-full bg-gamehub-bg border border-gamehub-muted text-gamehub-text px-4 py-3 rounded-md focus:border-gamehub-purple focus:ring-1 focus:ring-gamehub-purple focus:outline-none transition-all font-sans"
        {...props}
      />
    </div>
  );
  ...
  return (
    <div className="bg-gamehub-bg flex-1 min-h-screen font-sans pb-20">
      <div className="max-w-7xl mx-auto w-full px-6 py-8">
        ...
        <Link to="/shop" className="inline-flex items-center gap-2 text-gamehub-muted hover:text-[#ff79c6] transition-colors mb-8 text-sm font-bold uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            <form id="checkout-form" onSubmit={handlePlaceOrder} className="flex flex-col gap-8">
              ...
              <section className="bg-gamehub-surface/30 border border-gamehub-muted/30 rounded-lg p-6">
              ...
              <section className="bg-gamehub-surface/30 border border-gamehub-muted/30 rounded-lg p-6">
                <h2 className="font-pixel text-sm text-gamehub-text mb-6 flex items-center justify-between">
                  ...
                  <span className="flex items-center gap-1.5 text-[10px] font-sans text-gamehub-cyan font-bold bg-[#50fa7b]/10 px-3 py-1.5 rounded border border-[#50fa7b]/30 uppercase tracking-widest">
          ...
          <div className="lg:col-span-5">
            <div className="bg-gamehub-surface/50 rounded-lg p-6 lg:p-8 border-2 border-gamehub-purple sticky top-24 shadow-[0_0_15px_rgba(189,147,249,0.15)]">
              ...
              <div className="flex flex-col gap-5 mb-8 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                {cart.map(item => (
                  <div key={item.id} className="flex gap-4 items-center">
                    <div className="w-16 h-20 shrink-0 bg-gamehub-bg relative rounded border border-gamehub-muted overflow-hidden">
                      ...
                      <div className="absolute top-0 right-0 w-5 h-5 bg-gamehub-purple flex items-center justify-center text-[10px] font-bold text-black border-b border-l border-[#282a36]">
                        1
                      </div>
                    </div>
                    ...
                    <div className="flex items-center shrink-0">
                      <span className="text-gamehub-cyan font-bold text-sm">${item.price.toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>

              <div className="flex flex-col gap-3 text-sm font-bold border-t border-gamehub-muted/50 pt-5 mb-5 text-gamehub-text">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gamehub-cyan">FREE</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex justify-between items-end border-t border-gamehub-muted/50 pt-5 mb-8">
                <span className="font-pixel text-lg text-gamehub-text">TOTAL</span>
                <span className="text-3xl font-bold text-gamehub-cyan">${orderTotal.toFixed(2)}</span>
              </div>
              ...
              <button 
                ...
                className="w-full py-4 bg-gamehub-purple text-black font-bold tracking-widest uppercase text-base rounded hover:-translate-y-1 hover:bg-[#ff79c6] transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:bg-gamehub-purple cursor-pointer"
