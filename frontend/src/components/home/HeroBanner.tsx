...
export const HeroBanner = () => {
    ...
    return (
      <section className="relative w-full bg-theme-primary pt-0 pb-12">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="w-full h-[650px] sm:h-[550px] md:h-[460px] bg-gamehub-bg animate-pulse rounded-lg" />
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="relative w-full overflow-hidden bg-theme-primary pt-0 pb-12">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div 
          className="relative group rounded-lg overflow-hidden shadow-2xl flex flex-col md:flex-row bg-gamehub-bg min-h-[650px] sm:min-h-[550px] md:min-h-[460px]"
        ...
        >
          ...
          <button 
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 h-20 w-8 md:w-10 flex items-center justify-center bg-black/50 hover:bg-black/80 text-gamehub-text rounded-r-md opacity-0 group-hover:opacity-100 transition-all z-30"
          >
            <ChevronLeft className="w-8 h-8" />
          </button>
          <button 
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 h-20 w-8 md:w-10 flex items-center justify-center bg-black/50 hover:bg-black/80 text-gamehub-text rounded-l-md opacity-0 group-hover:opacity-100 transition-all z-30"
          >
          ...
          <div className="w-full md:w-[38%] p-6 flex flex-col h-full justify-between relative bg-gradient-to-r from-[#191A21] to-[#282a36]">
                    ...
                    <AnimatePresence mode="wait">
                      {tagsToRender.map((tag) => (
                        <motion.span 
                          ...
                          className="px-2 py-1 bg-gamehub-bg text-gamehub-muted text-xs rounded hover:bg-gamehub-surface hover:text-gamehub-text transition-colors cursor-pointer capitalize"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
            ...
            <div className="flex items-end justify-between mt-auto">
              ...
              <div className="flex items-center gap-3">
                {activeGame.price > 0 ? (
                  <div className="text-lg font-bold text-gamehub-cyan drop-shadow-[0_0_8px_rgba(80,250,123,0.3)]">
                    ${activeGame.price.toFixed(2)}
                  </div>
                ) : (
                  <div className="text-lg font-bold text-gamehub-cyan">
                    Free To Play
                  </div>
                )}
                <Link 
                  ...
                  className="px-3 py-1.5 bg-gamehub-purple text-gamehub-bg text-xs font-bold rounded hover:bg-white transition-colors"
                >
                  VIEW DETAILS
                </Link>
              </div>
            ...
            {games.map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  idx === currentIndex 
                    ? 'w-8 bg-gamehub-purple shadow-[0_0_10px_rgba(189,147,249,0.5)]' 
                    : 'w-4 bg-gamehub-surface hover:bg-gamehub-hover'
                }`}
