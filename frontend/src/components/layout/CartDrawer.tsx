      ...
      {isDrawerOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 h-full w-full max-w-md bg-gamehub-bg border-l-2 border-gamehub-purple/50 z-50 flex flex-col shadow-[-10px_0_30px_rgba(189,147,249,0.15)]"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gamehub-purple/30 relative">
              ...
              <button
                onClick={closeCart}
                className="p-2 text-gamehub-muted hover:text-[#ff5555] hover:bg-[#ff5555]/10 rounded-md transition-all duration-200"
              >
                <X className="w-6 h-6" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar">
              {cart.length === 0 ? (
                <div className="flex-grow flex flex-col items-center justify-center text-center py-12">
                  ...
                  <button
                    onClick={closeCart}
                    className="mt-6 text-sm font-bold text-[#ff79c6] hover:text-gamehub-purple transition-colors flex items-center gap-2"
                  >
                ...
                cart.map((item) => (
                  <motion.div
                    layout
                    key={item.id}
                    className="flex gap-4 p-2.5 rounded-lg border border-transparent hover:border-gamehub-muted/30 hover:bg-gamehub-surface/30 transition-all duration-200"
                  >
                    {/* Cover image */}
                    <div className="w-16 h-20 shrink-0 bg-black/40 overflow-hidden rounded border border-gamehub-muted/40">
                      <img
                    ...
                    <div className="flex flex-col flex-grow min-w-0 justify-between">
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <span className="text-sm font-bold text-gamehub-cyan">
                          ${item.price.toFixed(2)}
                        </span>
                        <button
                          onClick={() => removeFromCart(item.id)}
                          className="p-1.5 text-gamehub-muted hover:text-[#ff5555] hover:bg-[#ff5555]/10 rounded transition-all duration-200"
                          title="Remove item"
                        >
            ...
            {/* Footer */}
            {cart.length > 0 && (
              <div className="p-6 border-t border-gamehub-purple/30 bg-gamehub-bg relative shrink-0">
                <div className="flex justify-between items-center mb-4">
                  <span className="font-pixel text-xs text-gamehub-text tracking-widest uppercase">SUBTOTAL</span>
                  <span className="text-xl font-bold text-gamehub-cyan drop-shadow-[0_0_5px_rgba(80,250,123,0.3)]">
                    ${cartTotal.toFixed(2)}
                  </span>
                </div>
                ...
                <button 
                  ...
                  className="w-full block py-3.5 text-sm font-bold font-sans bg-gamehub-purple text-gamehub-bg tracking-widest uppercase rounded hover:-translate-y-0.5 hover:bg-[#ff79c6] active:translate-y-0 transition-all duration-200 flex items-center justify-center gap-2"
